...
async def create_log(log: LogIn):
    inserted_id = await insert_log(log_collection, log)
    log_doc = await get_log(log_collection, inserted_id)
    if not log_doc:
...
async def read_log(log_id: str):
    log = await get_log(log_collection, log_id)
    if not log:
...
async def update_log_by_id(log_id: str, log: LogIn):
    updated_count = await update_log(log_collection, log_id, log)
    if updated_count == 0:
...
async def delete_log_by_id(log_id: str):
    deleted_count = await delete_log(log_collection, log_id)
    if deleted_count == 0:
